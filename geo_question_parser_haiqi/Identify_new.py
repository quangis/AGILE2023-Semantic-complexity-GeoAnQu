...
# [SC][TODO] remane Grammar1
from geo_question_parser_haiqi.Grammar1.GeoAnQuLexer import GeoAnQuLexer
from geo_question_parser_haiqi.Grammar1.GeoAnQuParser import GeoAnQuParser
...
class QuestionParser:
    ...
    def write_type(self, coreDict):
        ...
        for cur_tag in coreDict['tag']:
            if cur_tag == 'distfield' or cur_tag == 'location' or cur_tag == 'allocation': #--------new---------
                ...
                if csign == 1:
                    ...
                    #     coreType['keyword'] = 'number of ' + self.result[conA_Reits[0]][int(conA_Reits[1])]
        ...
        #     coreDict['id'].insert(coreDict['tag'].index('roadData'), str(self.core_id))
        #     self.core_id += 1
    ...
    def geo_parser(self, sentence):
        ...
        try:
            ...
            # print('coreConTrans:', self.coreConTrans)
            ...
            if (tt == 'boolfield' and 'serviceobj' not in TypeDict['tag']) or tt == 'allocation' or tt == 'conAm':
                ...
                if TypeDict['tag'].index(tt) - 1 >= 0:
                    ...
                    if cur_tag == 'networkC':  # networkC -> object -> distField
                        ...
                        self.coreConTrans.setdefault('types', []).append({'type': 'object', 'id': str(self.core_id),
                                                                     'keyword': self.coreConTrans['types'][dist_index - 1][
                                                                         'keyword']})
                        # object -> distField
                        trans = {}
                ...
                if s_in - 2 >= 0 and TypeDict['tag'][s_in - 2] == 'destination':
                    if s_in - 3 >= 0 and TypeDict['tag'][
                        ...
                        self.coreConTrans.setdefault('types', []).append(self.new_type(trans_service['before'][0]))
                        self.core_id += 1
                    ...
                    self.coreConTrans.setdefault('types', []).append(self.new_type(trans_ser['before'][0]))
                    self.core_id += 1
                # duplicate?
                ...
                self.coreConTrans.setdefault('types', []).append(
                    {'type': 'objectquality', 'id': str(self.core_id),
                     'keyword': TypeDict['text'][TypeDict['tag'].index('extreDist')] + ' ' +
                                self.coreConTrans['types'][net_index]['keyword'], 'measureLevel': 'ratio'})
                self.core_id += 1
    ...
    def write_trans(self, coreTypeDict):
        try:
            ...
            if 'measure' in coreTypeDict['funcRole']:
                ...
                self.measureType = coreTypeDict['types'][meais[0]]
                ...
                if any('proportion' in e for e in coreTypeDict['types'][meais[0]]['text']) or any('eveconobjconpro' in e for e in coreTypeDict['types'][meais[0]]['text']):  #-------new---------
                    ...
                    if amount_id:
                            ...
                            if mea1is:
                                ...
                                if any('conamount' in e for e in coreTypeDict['types'][meais[0]]['text']):
                                    ...
                                    if 'id' not in coreTypeDict['types'][con_meais[0]]:  # noise larger than 70 db
                                        ...
                                        transcross = {}
                                        transcross['before'] = [str(self.core_id - 1), str(
                                            self.core_id)]  # self.core_id-1 = field covamount, self.core_id = extent covamount
                                        transcross['after'] = [coreTypeDict['types'][meais[0]]['id'][-1]]
                                        transcross['key'] = self.coreConTrans['extent'][0]
                                        coretrans.append(transcross)
                                        self.core_id += 1
                                        ...
                                        transcross['before'] = [str(self.core_id - 1), str(self.core_id)]
                                        transcross['after'] = [coreTypeDict['types'][meais[0]]['id'][-1]]
                                        transcross['key'] = self.coreConTrans['extent'][0] #---------new-----
                                        coretrans.append(transcross)
                                        self.core_id += 1
                                else:
                                    ...
                                    transcross['before'] = [str(self.core_id - 1)]
                                    transcross['after'] = [str(self.core_id)]
                                    transcross['key'] = self.coreConTrans['extent'][
                        ...
                        if con_meais and not supis:
                            ...
                            coretrans.append(transcross)
                            # add new proportion type in self.coreConTrans[types]
                    ...
                    if not supis and not con_meais:
                        if 'loc' in coreTypeDict['types'][meais[0]]['text'][-1] and 'weight' in coreTypeDict:
                            ...
                            #     cov_trans[0]['key'] = self.coreConTrans['extent'][0]
                    ...
                    if supis and not con_meais:
                        ...
                        for ci in con_meais:
                            ...
                            #     trans_compR['after'] = [str(self.core_id)]
                            #     coretrans.append(trans_compR)
                            #     self.coreConTrans.setdefault('types', []).append(new_type(coreTypeDict['types'][ci]['id'][0]))
                            #     self.core_id += 1
                            #     transcross = {}
                            #     transcross['before'] = [trans_compR['after'][0], coreTypeDict['types'][meais[0]]['id'][0]]
                            #     transcross['after'] = [str(self.core_id)]
                            #     coretrans.append(transcross)
                            #     self.coreConTrans.setdefault('types', []).append(new_type(coreTypeDict['types'][meais[0]]['id'][0]))
                            #     self.core_id += 1
...
# [SC][TODO] addition
ptypePath_ex = dictPath / "place_type_ex.txt"
corePath_ex = dictPath / "coreConceptsML_ex.txt"
networkPath_ex = dictPath / "network_ex.txt"
cn_ex = dictPath / "cn_ex.txt"
patterns_ex = dictPath / "patterns_ex.txt"
humanWord_ex = dictPath / "humanWord_ex.txt"
...
# [SC][TODO] addition
patterns.extend(list(nlp(line.strip()) for line in open(patterns_ex, encoding="utf-8")))
cn.update(set(line.strip() for line in open(cn_ex, encoding="utf-8")))
humanWords.update(set(line.strip() for line in open(humanWord_ex, encoding="utf-8")))
...
# [SC][TODO] addition
coreCon_dict_ex = load_ccdict(corePath_ex)
coreCon_dict['text'].extend(coreCon_dict_ex['text'])
coreCon_dict['tag'].extend(coreCon_dict_ex['tag'])
coreCon_dict['measureLevel'].extend(coreCon_dict_ex['measureLevel'])
pt_set.update(set(line.strip() for line in open(ptypePath_ex, encoding="utf-8")))
networkSet.update(set(line.strip() for line in open(networkPath_ex, encoding="utf-8")))

pos = []
units = {'DB', 'Db', 'db', 'dB', 'decibel', 'meters'}
...
# [SC][DELETE] these logging files is for development only; remove from production
logPath = rootPath / "ParserLog"
errorQuestionFilePath = logPath / 'error.txt'
afterplace = logPath / 'afterplace.txt'
afterentity = logPath / 'afterentity.txt'
afterconcept = logPath / 'afterconcept.txt'
afterparser = logPath / 'afterparser.txt'
